...
fn handle_key_tui(app: &mut App, key: crossterm::event::KeyEvent, in_command: &mut bool) -> bool {
    ...
    match app.overlay {
        ...
        Overlay::FleetGrid => {} // native-only feature; the TUI fallback ignores it.
        ...
        Overlay::Hosts => {}     // native-only feature; the TUI fallback ignores it.
    ...
    if *in_command {
        *in_command = false;
        match key.code {
            ...
            KeyCode::Char('x') if !app.tabs.is_empty() => {
